...
public class FactionCommand implements CommandExecutor {
        ...
        switch (sub) {
            ...
            case "create" -> {
                ...
                if (plugin.getFactionManager().factionExists(fName)) {
                    sender.sendMessage(ChatColor.RED + "Fazione gia esistente!");
                    return true;
                }
                ...
                if (plugin.getFactionManager().isPlayerInFaction(leader.getUniqueId())) {
                    sender.sendMessage(ChatColor.RED + leader.getName() + " e gia in una fazione!");
                    return true;
                }
            ...
            case "delete" -> {
                ...
                if (args.length < 2) {
                    sender.sendMessage(ChatColor.RED + "Uso: /f delete <nomefazione>");
                    return true;
                }
                String fName = args[1];
                Faction faction = plugin.getFactionManager().getFaction(fName);
                ...
                // Rimuovi tutti i membri dalla fazione
                for (UUID memberUUID : faction.getMembers().keySet()) {
                    plugin.getFactionManager().removePlayerFromFaction(memberUUID);
                    Player member = plugin.getServer().getPlayer(memberUUID);
                    if (member != null) {
                        member.sendMessage(ChatColor.RED + "La tua fazione " + fName + " e stata eliminata dall admin!");
                    }
                }

                plugin.getFactionManager().deleteFaction(fName);
                sender.sendMessage(ChatColor.GREEN + "Fazione " + fName + " eliminata!");
            }

            // === ADMIN: /f kick <nomefazione> <nomeplayer> ===
            case "kick" -> {
                if (!sender.hasPermission("italiafactions.admin")) {
                    sender.sendMessage(ChatColor.RED + "Non hai i permessi!");
                    return true;
                ...
                if (args.length < 3) {
                    sender.sendMessage(ChatColor.RED + "Uso: /f kick <nomefazione> <nomeplayer>");
                    return true;
                }
                String fName = args[1];
                ...
                if (!faction.hasMember(target.getUniqueId())) {
                    sender.sendMessage(ChatColor.RED + playerName + " non e in questa fazione!");
                    return true;
                }

                plugin.getFactionManager().removePlayerFromFaction(target.getUniqueId());
                sender.sendMessage(ChatColor.GREEN + playerName + " rimosso dalla fazione " + fName + "!");
                target.sendMessage(ChatColor.RED + "Sei stato rimosso dalla fazione " + fName + " dall admin!");
            }
            ...
            case "mappa" -> {
                ...
                plugin.getGUIManager().openMappaTerritori(admin);
            }
                ...
                if (setType.equals("territorio")) {
                    ...
                    if (plugin.getFactionManager().isPlayerInFaction(target.getUniqueId())) {
                        ...
                        if (!currentFaction.equalsIgnoreCase(fName)) {
                            sender.sendMessage(ChatColor.RED + playerName + " e gia in un'altra fazione!");
                            return true;
                        }
                    ...
                    plugin.getFactionManager().saveFactions();
                    sender.sendMessage(ChatColor.GREEN + playerName + " e ora " + rank.name() + " nella fazione " + fName);
            ...
            case "chat" -> {
                ...
                if (ChatListener.factionChatEnabled.contains(uuid)) {
                    ...
                    player.sendMessage(ChatColor.YELLOW + "Chat fazione disattivata.");
                } else {
                    ChatListener.factionChatEnabled.add(uuid);
                    Faction faction = plugin.getFactionManager().getFactionByPlayer(uuid);
                    player.sendMessage(ChatColor.YELLOW + "Chat fazione attivata [" + faction.getName() + "].");
                }
            }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        if (sender.hasPermission("italiafactions.admin")) {
            ...
            sender.sendMessage(ChatColor.RED + "/f delete <nome> - Elimina fazione");
            sender.sendMessage(ChatColor.RED + "/f kick <fazione> <player> - Licenzia player");
            sender.sendMessage(ChatColor.RED + "/f mappa - Mappa territori");
